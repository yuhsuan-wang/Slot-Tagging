...
class Vocab:
    ...
    def encode_batch(
        ...
        batch_ids = [self.encode(tokens) for tokens in batch_tokens] #所有句子轉id
        to_len = max(len(ids) for ids in batch_ids) if to_len is None else to_len #最長的句子的長度
        padded_ids = pad_to_len(batch_ids, to_len, self.pad_id) #做padding
        return padded_ids
